...
- ajuste de diseno aplicado a `why_it_supports`: la explicacion rica vuelve a depender principalmente del LLM grounded en `S6.5`; el fallback backend deja de intentar inferir semantica por heuristica y pasa a una formulacion neutra de seguridad, evitando resultados ambiguos como tokens vacios o referencias a `section unknown`
- refinacion adicional del prompt de `S6.5`: `best_supporting_evidence` ahora se instruye como evidencia curada y no exhaustiva; prioriza soporte directo, luego parcial defendible, evita redundancia y desalienta incluir snippets contextuales si no agregan una senal distinta
- refinacion adicional de trazabilidad en `S6.5`: `best_supporting_evidence` debe salir unicamente de `best_evidence` o `accepted_matches` de `S6` y preservar `source_ref`, `block_title` y `section` exactamente como vengan aguas arriba; `unknown` se conserva como valor valido y ya no debe degradarse a strings vacios
